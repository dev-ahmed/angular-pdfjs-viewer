...
'              </button>\n' +
'\n' +
'              <a id="download" class="toolbarButton download hiddenMediumView" download title="Download" tabindex="34" data-l10n-id="download">\n' +
'                <span data-l10n-id="download_label">Download</span>\n' +
'              </a>\n' +
'\n' +
					...
					if($attrs.download){
							...
							$http.get(pdfLink , {responseType: 'arraybuffer'}).then(function(response){
								if(response.status ==200){
									...
									var temp = response.config.url.split('/');
